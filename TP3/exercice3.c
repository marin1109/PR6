...
void client(int client_socket) {
    ...
    while (tentatives < 20) {
        ...
        // Vérifier si la donnée reçue est un nombre valide
        char *endptr;
        ...
        if (tmp == n) {
            ...
            snprintf(buffer, BUFFER_SIZE, "PLUS | Tentatives restantes : %d\n", 20 - tentatives - 1);
            send(client_socket, buffer, strlen(buffer), 0);
        } else {
            snprintf(buffer, BUFFER_SIZE, "MOINS | Tentatives restantes : %d\n", 20 - tentatives - 1);
            send(client_socket, buffer, strlen(buffer), 0);
        }
        tentatives++;
    }
